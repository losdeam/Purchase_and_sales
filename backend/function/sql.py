...
def get_values(data, key, sheet):
    ...
    if result:
        ...
        return None
def get_values_time(data:any, key:str, sheet:str):
    '''
    根据所给的data值在sheet的key字段中寻找所有符合的记录
    return : 字典格式的记录,不存在则返回None
    '''
    ...
    if result:
        ...
        return None
def update_data(data:any, key:str, sheet:str,new_data:dict):
    data_result = []
    stmt = db.select(dict_sheet[sheet]).where(getattr(dict_sheet[sheet], key) == data)
    result = db.session.execute(stmt).scalar_one_or_none()
    if result != None:
        ...
        db.session.commit()
    return data_result
def delete_value(data:any, key:str, sheet:str):
    '''
