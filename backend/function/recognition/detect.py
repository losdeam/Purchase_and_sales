...
def get_good_data(detect_data):
    ...
    for goods_id in detect_data['label']:
        if goods_id in temp_dict:
            ...
            goods_data = redis_client.hget('good_data', goods_id)
            goods_data_dict  = json.loads(goods_data)
            temp_dict[goods_id] = {}
            temp_dict[goods_id]['goods_id'] =  goods_id
            temp_dict[goods_id]['goods_name'] =  str(goods_data_dict['good_name'])
            temp_dict[goods_id]['goods_count'] = 1
            temp_dict[goods_id]['goods_price'] = float(goods_data_dict['good_price_retail'])
    for good_id in temp_dict:
        recognize_data.append(temp_dict[good_id])
